...
@Singleton
class AuthDeadboltHandler @Inject()(authz: Authz, errorHandler: HttpErrorHandler,sessionCfg: SessionCookieBaker)(implicit ec: ExecutionContext) extends DeadboltHandler{
  ...
  override def getSubject[A](request: AuthenticatedRequest[A]): Future[Option[Subject]] = {
    val cookie = request.cookies.get(sessionCfg.COOKIE_NAME)
    val session:Session= new Session(sessionCfg.decodeCookieToMap(cookie).asJava)
    val subject = authz.getSubject(session)
  ...
  /**
    ...
    * @return the action
    */
  override def onAuthFailure[A](request: AuthenticatedRequest[A]): Future[Result] = errorHandler.onClientError(request,401,"Authenticated Failure")
  /**
