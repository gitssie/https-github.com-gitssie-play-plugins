...
class DefaultPasswordService @Inject() (secret:SecretConfiguration) extends PasswordService {
  ...
  def encryptAES(value: String): String = {
    secret.provider.map(secret => encryptAES(value, secret.substring(0, 16))).getOrElse {
      throw new PlayException("Configuration error", "Missing play.http.secret.key")
    }
  }
  ...
  def decryptAES(value: String): String = {
    secret.provider.map(secret => decryptAES(value, secret.substring(0, 16))).getOrElse {
      throw new PlayException("Configuration error", "play.http.secret.key")
    }
  }
