...
private [deadbolt] class DeadboltSubject(subject:play.api.deadbolt.Subject) extends Subject{
  ...
  override lazy val roles :List[Role]= subject.roles.map(new DeadboltRole(_))

  override lazy val permissions : List[Permission]= subject.permissions.map(new DeadboltPermission(_))
}

private [deadbolt] class DeadboltRole(role:play.api.deadbolt.Role) extends Role{
  override def name = role.name
}
