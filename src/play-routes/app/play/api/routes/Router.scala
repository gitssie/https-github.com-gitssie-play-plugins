...
/**
  ...
  */
@Singleton
class Router @Inject()(env:Environment,conf:Configuration,inject:Injector) extends PlayRouter {
  ...
  val includes:Seq[PlayRouter] = {
    ...
    routesClassNames.map(line => {
        var idx = line.trim.indexOf(pathSplitAt)
        if(idx >= 0){
          val part = line.substring(0,idx).trim
          val end = line.substring(idx+pathSplitAt.length,line.length).trim
      ...
      try {
        val clazz = Reflect.getClass[PlayRouter](entry._2, env.classLoader)
